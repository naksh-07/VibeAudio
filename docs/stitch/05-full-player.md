# Stitch Screen Prompt 05 — Full Player & Book Detail (`#view-player`)
...
## 1. Screen Objective
Synthesize the primary immersion sanctuary of VibeAudio. When a listener expands the player or initiates playback, this screen presents the book cover artwork with a luminous ambient bloom, literary typography, a high-precision hardware transport deck, chapter playlists, and timestamp-locked notes.
...
* **Header Card Presentation (`.player-header-card`):**
  ...
  * 180px wide cover with authentic 2:3 vertical aspect ratio and `14px` squircle corners.
  * Title in `Newsreader` (36px bold, `#1D1D1F`), author in `#6E6E73`.
  * CTA Row: Primary solid button `#C64E00` with white text, secondary outline buttons.
* **Transport Deck (`.player-transport-deck`):**
  ...
  * Scrubber Track: 6px height, `rgba(0, 0, 0, 0.06)` track, solid `#C64E00` fill, 18px pure white circular thumb with subtle drop shadow (`0 2px 8px rgba(0, 0, 0, 0.2)`).
  * Main Play Button (`#play-btn`): 56×56px round button in `#C64E00` with `#FFFFFF` play/pause glyph (verified 4.67:1 WCAG AA contrast) and warm ambient glow.
...
* **Chapter Playlist & Moments (Two-Column):**
  * Left: Chapter items in `#F2F2F7` background; active playing chapter highlighted in soft amber `rgba(198, 78, 0, 0.08)` with 3px left `#C64E00` accent border.
...
* `Header Surface`: Solid `#FFFFFF` (`var(--color-surface-1)`)
* `Primary Play Button`: `#C64E00` solid, pure white glyph, 56×56px circle, shadow `0 6px 20px rgba(198, 78, 0, 0.28)`.
* `Scrubber Track`: 6px height, active fill `#C64E00`.
* `Timecode Monospace`: `JetBrains Mono` with `tabular-nums`.
