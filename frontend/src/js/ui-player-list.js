...
export function renderChapterList(book) {
    ...
    chaptersToShow.forEach((chapter, idx) => {
        ...
        if (chapter.section && chapter.section !== lastSection) {
            ...
            sectionHeader.innerHTML = `<span>${escapeHTML(chapter.section)}</span><svg class="vibe-icon vibe-icon-sm ${isOpen ? 'rotate' : ''}" aria-hidden="true"><use href="#icon-chevron-down"></use></svg>`;
        ...
        const statusIconName = isCurrent
            ? 'equalizer'
            : isCompleted
                ? 'check-circle'
                : isResumePoint
                    ? 'history'
                    : 'play';
        const statusIconExtra = isCurrent ? ' vibe-icon-accent' : isCompleted ? ' vibe-icon-success' : '';
        ...
        item.innerHTML = `
            ...
            <div class="chapter-status"><svg class="vibe-icon vibe-icon-xs${statusIconExtra}" aria-hidden="true"><use href="#icon-${statusIconName}"></use></svg></div>
        `;
        item.onclick = () => {
            loadBook(book, idx);
