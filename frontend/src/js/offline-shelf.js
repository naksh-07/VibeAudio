...
async function getStorageRoot() {
    ...
    try {
        ...
        console.warn('OPFS storage root is unavailable.', error);
        return null;
    }
}
...
async function ensureOpfsDirectory(pathSegments) {
    ...
    try {
        ...
        console.warn('Unable to ensure OPFS directory path.', error);
        return null;
    }
}
...
async function writeBlobToOpfs(pathSegments, blob) {
    try {
        ...
        console.warn('OPFS blob write failed. Falling back to IndexedDB.', error);
        return null;
    }
}
...
async function deleteBlobFromOpfs(path) {
    ...
    try {
        ...
        // NotFoundError or permission errors are expected if file is already deleted
        return false;
    }
}
...
async function readBlobFromOpfs(path) {
    ...
    try {
        ...
        console.warn('OPFS file read failed for path:', path, error);
        return null;
    }
}
...
async function updateStorageStats(userId = getActiveUserId()) {
    ...
    if (typeof window !== 'undefined' && window.VibePWA?.updateAppBadge) {
        window.VibePWA.updateAppBadge(stats.pendingJobs).catch(() => {});
    }
    return stats;
}
...
async function getNextRunnableJob(userId = getActiveUserId()) {
    ...
    return jobs
        .filter((job) => {
            if (job.status === OFFLINE_STATES.queued) return true;
            if (job.status === OFFLINE_STATES.downloading) return !activeDownloadController;
            if (job.status === OFFLINE_STATES.failed) return toSafeNumber(job.retries, 0) < MAX_JOB_RETRIES;
            return false;
        })
    ...
    try {
        ...
        if (prefersOpfs) {
            ...
            try {
                ...
                console.warn('OPFS storage attempt failed. Falling back to IndexedDB blob.', opfsErr);
                opfsPath = null;
            }
            if (!opfsPath) {
        ...
        const currentRetries = toSafeNumber(job.retries, 0);

        let willRetry = navigator.onLine && !isAborted && !isQuota && currentRetries < MAX_JOB_RETRIES;
        let nextStatus = willRetry ? OFFLINE_STATES.failed : (isOffline || isAborted ? OFFLINE_STATES.queued : OFFLINE_STATES.failed);
        ...
        if (isOffline) {
            ...
            errorReason = 'Storage is full on this device. Free some space and try again.';
        } else if (error instanceof TypeError) {
            errorReason = 'Network connection interrupted';
        } else if (error?.message) {
            errorReason = error.message;
        }

        const retryIndex = Math.min(currentRetries, JOB_RETRY_DELAYS_MS.length - 1);
        const nextDelay = (isOffline || isAborted) ? 0 : JOB_RETRY_DELAYS_MS[retryIndex];
        const nextAttemptAt = nextDelay > 0
        ...
        await putRecord(STORE_NAMES.jobs, {
            ...job,
            status: nextStatus,
            retries: willRetry ? currentRetries + 1 : currentRetries,
            errorReason,
            updatedAt: new Date().toISOString(),
            nextAttemptAt
        });
...
export async function getBackgroundFetchRegistration() {
    ...
    try {
        ...
        if (registration && 'backgroundFetch' in registration && typeof registration.backgroundFetch.fetch === 'function') {
            return registration;
        }
    } catch (_) {}
    return null;
...
function buildBackgroundFetchJobId(userId, bookId, lang, chapterIndex) {
    return `vibe-bf-${encodeURIComponent(userId)}-${encodeURIComponent(bookId)}-${normalizeLang(lang)}-${Number(chapterIndex)}`;
}

async function tryBackgroundFetchForChapter(book, chapter, lang, chapterIndex) {
    const registration = await getBackgroundFetchRegistration();
    if (!registration) return false;

    const userId = getActiveUserId();
    const jobId = buildBackgroundFetchJobId(userId, book.bookId, lang, chapterIndex);

    try {
        const existing = await registration.backgroundFetch.get(jobId);
        if (existing) {
            return true;
        }
    } catch (_) {}
    ...
    try {
        const title = `${book.title || 'Audiobook'} · ${chapter.name || `Part ${Number(chapterIndex) + 1}`}`;
        const icons = book.cover ? [{ src: book.cover, sizes: '192x192' }] : [];
        const downloadTotal = toSafeNumber(chapter.sizeBytes, 0);

        await registration.backgroundFetch.fetch(jobId, [chapter.url], {
            title,
            icons,
            downloadTotal: downloadTotal > 0 ? downloadTotal : undefined
        });

        await upsertJobRecord(book, lang, chapterIndex, OFFLINE_STATES.downloading);
        const currentRecord = await getRecord(STORE_NAMES.chapters, chapterRecordId(userId, book.bookId, lang, chapterIndex));
        if (currentRecord) {
            await putRecord(STORE_NAMES.chapters, {
                ...currentRecord,
                status: OFFLINE_STATES.downloading,
        ...
        emitOfflineChange('job-started', { bookId: book.bookId, lang: normalizeLang(lang), chapterIndex });
        return true;
    } catch (err) {
        console.warn('Background fetch initiation failed, falling back to foreground queue.', err);
        return false;
    }
}
...
export async function queueChapterDownload(book, chapter, lang, options = {}) {
    ...
    tryBackgroundFetchForChapter(book, chapter, normalizedLang, chapterIndex).then((started) => {
        if (!started) {
...
export async function removeOfflineChapter(bookId, lang, chapterIndex) {
    const userId = getActiveUserId();
    const normalizedLang = normalizeLang(lang);
    const chapterId = chapterRecordId(userId, bookId, normalizedLang, chapterIndex);
    const jobId = jobRecordId(userId, bookId, normalizedLang, chapterIndex);
    ...
    try {
        const registration = await getBackgroundFetchRegistration();
        if (registration) {
            const bgJobId = buildBackgroundFetchJobId(userId, bookId, normalizedLang, chapterIndex);
            const bgFetch = await registration.backgroundFetch.get(bgJobId);
            if (bgFetch) {
                await bgFetch.abort();
            }
        }
    ...
    await updateStorageStats();
    emitOfflineChange('chapter-removed', { bookId, lang: normalizedLang, chapterIndex });
    return true;
}
...
export async function clearAllOfflineDownloads() {
    ...
    await Promise.allSettled(chapterRecords.map((record) => removeOfflineChapter(record.bookId, record.lang, record.chapterIndex)));
    await Promise.allSettled(bookRecords.map((record) => deleteRecord(STORE_NAMES.books, record.id)));
    await Promise.allSettled(jobRecords.map((record) => deleteRecord(STORE_NAMES.jobs, record.id)));
    await updateStorageStats();
...
const SUPPORTED_IMPORT_EXTENSIONS = ['.m4b', '.mp3'];
const SUPPORTED_IMPORT_MIMES = ['audio/mp4', 'audio/mpeg', 'audio/x-m4b', 'audio/x-mp3'];
const MAX_IMPORT_FILE_SIZE = 2 * 1024 * 1024 * 1024; // 2 GB

export function validateImportableAudioFile(file) {
    if (!file || typeof file !== 'object') {
        return { valid: false, error: 'No audio file was provided.' };
    }

    const name = String(file.name || '').toLowerCase();
    const size = toSafeNumber(file.size, 0);
    ...
    if (size > MAX_IMPORT_FILE_SIZE) {
        return { valid: false, error: 'Audio file exceeds 2 GB limit.' };
    }

    const hasValidExtension = SUPPORTED_IMPORT_EXTENSIONS.some((ext) => name.endsWith(ext));
    const hasValidMime = SUPPORTED_IMPORT_MIMES.includes(type) || type.startsWith('audio/');

    if (!hasValidExtension && !hasValidMime) {
        return { valid: false, error: 'Unsupported file format. Please select an .mp3 or .m4b audiobook file.' };
    }

    return {
        valid: true,
        extension: name.endsWith('.m4b') ? '.m4b' : '.mp3',
        mimeType: type || (name.endsWith('.m4b') ? 'audio/mp4' : 'audio/mpeg'),
        error: ''
    };
}

export async function importAudiobookFile(file, options = {}) {
    bindOfflineLifecycle();
    ...
    const rawName = String(file.name || 'Local Audiobook');
    const cleanTitle = rawName
        .replace(/\.(m4b|mp3)$/i, '')
        .replace(/[-_]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim() || 'Local Audiobook';
        ...
        chapters: [
            {
                ...
                mimeType,
                sizeBytes,
                versionTag: `local-${file.size}-${file.lastModified || 0}`,
                checksum: ''
            }
        ],
        chapters_en: []
    ...
    await ensureBookRecord(bookRecord, 'hi');
    ...
    if (prefersOpfs) {
        const path = buildOpfsPath(userId, bookId, 'hi', 0);
        try {
            opfsPath = await writeBlobToOpfs(path, file);
        } catch (opfsErr) {
            console.warn('OPFS storage attempt failed for imported file. Falling back to IndexedDB.', opfsErr);
            opfsPath = null;
        }
    ...
    const chapterId = chapterRecordId(userId, bookId, 'hi', 0);
    const existingChapter = await getRecord(STORE_NAMES.chapters, chapterId);
    ...
    const chapterRecord = {
        ...
        opfsPath,
        fallbackBlob,
        downloadedAt: existingChapter?.downloadedAt || now,
        validatedAt: now,
    ...
    await putRecord(STORE_NAMES.chapters, chapterRecord);
    await updateStorageStats(userId);

    emitOfflineChange('file-imported', {
        bookId,
        book: bookRecord,
