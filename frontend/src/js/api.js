...
import {
    ...
    setCatalogSnapshot,
    setSyncStatus,
    syncAndMigratePendingQueue,
    upsertPendingProgress
} from './user-data.js';
import { requestProgressBackgroundSync } from './pwa.js';
...
function bindSyncLifecycle() {
    ...
    window.addEventListener('vibe-background-sync-complete', (event) => {
        syncAndMigratePendingQueue().then(() => {
            invalidateProgressCache();
            const remaining = getPendingProgressEntries();
            setSyncStatus(remaining.length ? SYNC_STATES.pending : SYNC_STATES.synced, {
                pendingCount: remaining.length,
                lastSuccessfulSyncAt: event.detail?.success ? new Date().toISOString() : null,
                reason: ''
            });
        }).catch(() => {});
    });
...
function getLocalProgressEntries() {
    ...
    for (let i = 0; i < localStorage.length; i += 1) {
        ...
        try {
            ...
            const bookKey = String(normalized.bookId);
            const isUserEntry = activeUserId && normalized.userId === activeUserId;
            const isGuestEntry = !normalized.userId || normalized.userId === 'guest';

            if (isUserEntry) {
                entriesByBook.set(bookKey, normalized);
            } else if (isGuestEntry && !entriesByBook.has(bookKey)) {
                entriesByBook.set(bookKey, normalized);
            }
        } catch (error) {
...
export async function saveUserProgress(bookId, chapterIndex, currentTime, totalDuration, options = {}) {
    ...
    const userId = authUserId || getUserId() || 'guest';
    ...
    const shouldSendToServer = Boolean(authUserId)
    ...
    if (!navigator.onLine) {
        ...
        requestProgressBackgroundSync().catch(() => {});
        return normalizedPayload;
    }
...
export async function flushPendingProgressQueue() {
    ...
    await syncAndMigratePendingQueue().catch(() => {});
...
export async function fetchUserProgress(options = {}) {
    ...
    if (navigator.onLine) {
        ...
        try {
            const response = await fetch(`${APP_CONFIG.getProgressUrl}?userId=${encodeURIComponent(authUserId)}`);
            if (!response.ok) {
