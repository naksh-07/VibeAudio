...
export async function getSignedInUser() {
    if (!navigator.onLine) {
        const localId = localStorage.getItem(STORAGE_KEYS.userId);
        if (localId) {
            return {
                id: localId,
                firstName: localStorage.getItem(STORAGE_KEYS.userName) || 'Vibe Listener',
                fullName: localStorage.getItem(STORAGE_KEYS.userName) || 'Vibe Listener'
            };
        }
        return null;
    }
    ...
    try {
        ...
        console.warn('Clerk auth initialization skipped or failed. Falling back to cached local identity.', error);
        const localId = localStorage.getItem(STORAGE_KEYS.userId);
        if (localId) {
            return {
                id: localId,
...
export async function ensureSignedInOrRedirect(redirectUrl) {
    if (!navigator.onLine) {
        const localId = localStorage.getItem(STORAGE_KEYS.userId);
        if (localId) {
            return { id: localId, firstName: localStorage.getItem(STORAGE_KEYS.userName) || 'Vibe Listener' };
        }
        return null;
    }
    ...
    try {
        ...
        console.warn('Auth verification failed. Allowing local session.', error);
        const localId = localStorage.getItem(STORAGE_KEYS.userId);
        if (localId) {
            return { id: localId, firstName: localStorage.getItem(STORAGE_KEYS.userName) || 'Vibe Listener' };
        }
